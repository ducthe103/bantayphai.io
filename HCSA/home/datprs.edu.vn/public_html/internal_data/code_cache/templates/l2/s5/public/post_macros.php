<?php
// FROM HASH: e438d4ad3886296243b084860c799e27
return array('macros' => array('post' => function($__templater, array $__arguments, array $__vars)
{
	...
	$__finalCompiled .= '
	<article class="message message--post js-post js-inlineModContainer ' . ((($__vars['post']['position'] % $__vars['xf']['options']['messagesPerPage']) == 0) ? 'is-first' : '') . ' ' . ($__vars['isIgnored'] ? 'is-ignored' : '') . ' ' . ($__templater->method($__vars['post'], 'isUnread', array()) ? ' is-unread' : '') . '"
...
<a href="' . $__templater->fn('link', array('threads/reply', $__vars['thread'], array('tag' => $__vars['post']['username'], ), ), true) . '"
   title="' . $__templater->filter('Insert username', array(array('for_attr', array()),), true) . '"
   class="actionBar-action actionBar-action--tag"
   data-username="' . $__templater->escape($__vars['post']['username']) . '"	
   data-xf-click="userTag">' . 'Tag' . '
</a>
											';
	}
	$__compilerTemp3 .= '
