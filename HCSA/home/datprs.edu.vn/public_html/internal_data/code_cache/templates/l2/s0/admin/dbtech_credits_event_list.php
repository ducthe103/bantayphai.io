<?php
// FROM HASH: dbe4cfc6a5284ba644b63fca572f5164
...
';
	...
	' . $__templater->button('Add event', array(
		'href' => $__templater->fn('link', array('dbtech-credits/events/add', ), false),
		'overlay' => 'true',
		'icon' => 'add',
	), '', array(
	)) . '
...
';
	...
	if ($__templater->isTraversable($__vars['currencies'])) {
		foreach ($__vars['currencies'] AS $__vars['currencyId'] => $__vars['currencyName']) {
			$__compilerTemp1 .= '
				<a href="' . $__templater->fn('link', array('dbtech-credits/events', null, array('currency_id' => $__vars['currencyId'], ), ), true) . '"
	...
	if ($__templater->isTraversable($__vars['events'])) {
		foreach ($__vars['events'] AS $__vars['event']) {
			$__compilerTemp2 .= '
				...
				'href' => $__templater->fn('link', array('dbtech-credits/events/edit', $__vars['event'], ), false),
			...
			array(
				'href' => $__templater->fn('link', array('dbtech-credits/events/delete', $__vars['event'], ), false),
				'tooltip' => 'Xóa' . ' ',
				'_type' => 'delete',
				'html' => '',
			))) . '
...
', array(
		'action' => $__templater->fn('link', array('dbtech-credits/events/toggle', ), false),
		'class' => 'block',
		'ajax' => 'true',
	));
