	...
	<li data-id="' . $__templater->escape($__vars['shout']['shout_id']) . '"' . ($__vars['rowClass'] ? ((' class="' . $__templater->escape($__vars['rowClass'])) . '"') : '') . '>
		...
		';
			...
			if ($__templater->method($__vars['shout'], 'canEdit', array())) {
				$__compilerTemp1 .= '
						<a href="' . $__templater->fn('link', array('shoutbox/edit', $__vars['shout'], ), true) . '" data-xf-click="overlay" title="' . $__templater->filter('Sửa', array(array('for_attr', array()),), true) . '">' . $__templater->fontAwesome('fa-edit', array(
				)) . '</a>
					';
			}
			$__compilerTemp1 .= '
			...
			if ($__templater->method($__vars['shout'], 'canDelete', array())) {
				$__compilerTemp1 .= '
						<a href="' . $__templater->fn('link', array('shoutbox/delete', $__vars['shout'], ), true) . '" data-xf-click="overlay" title="' . $__templater->filter('Xóa', array(array('for_attr', array()),), true) . '">' . $__templater->fontAwesome('fa-trash', array(
				)) . '</a>
					';
			}
			$__compilerTemp1 .= '
			...
			if ($__templater->method($__vars['shout'], 'canBan', array())) {
				...
				if ($__vars['isBanned']) {
					$__compilerTemp1 .= '
							<a href="' . $__templater->fn('link', array('shoutbox/unban', null, array('user_id' => $__vars['shout']['shout_user_id'], ), ), true) . '" data-xf-click="overlay" title="' . $__templater->filter('Bỏ cấm túc', array(array('for_attr', array()),), true) . '">' . $__templater->fontAwesome('fa-user-minus', array(
					)) . '</a>
						';
				} else {
					$__compilerTemp1 .= '
							<a href="' . $__templater->fn('link', array('shoutbox/ban', null, array('user_id' => $__vars['shout']['shout_user_id'], ), ), true) . '" data-xf-click="overlay" title="' . $__templater->filter('Cấm', array(array('for_attr', array()),), true) . '">' . $__templater->fontAwesome('fa-user-times', array(
					)) . '</a>
						';
				}
				$__compilerTemp1 .= '
