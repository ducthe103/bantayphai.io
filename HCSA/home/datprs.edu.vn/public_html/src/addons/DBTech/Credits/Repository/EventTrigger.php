...
class EventTrigger extends Repository
{
	 ...
	 * @throws \Exception
	 */
	public function getEventTriggers($onlyActive = false, $onlyWithEvents = false)
	{
		$eventTriggers = new ArrayCollection($this->getHandlers());
		
		if ($onlyActive)
		{
			$eventTriggers = $eventTriggers->filter(function(AbstractHandler $eventTrigger)
			{
		...
		if ($onlyWithEvents)
		{
			...
			$eventTriggers = $eventTriggers->filter(function(AbstractHandler $eventTrigger) use ($events, $onlyActive)
			{
				if ($onlyActive)
				{
				...
				$activeEvents = $events->filter(function(Event $event) use ($eventTrigger)
				{
	...
	public function getRebuildableEventTriggers()
	{
		...
		return $eventTriggers->pluck(function(AbstractHandler $eventTrigger)
		{
	...
	public function getRebuildableEventTriggerPairs()
	{
		...
		$arr = $eventTriggers->pluck(function(AbstractHandler $eventTrigger)
		{
	 ...
	 * @return array
	 */
	public function getEventTitlePairs($onlyActive = false, $forFullView = false)
	{
		...
		$arr = $events->pluck(function(Event $e, $k) use ($forFullView)
		{
			$title = $e->getTitle();
	 ...
	 * @throws \Exception
	 */
	public function getEventTriggerTitlePairs($filterActive = false, $onlyWithEvents = false)
	{
		$arr = $this->getEventTriggers($filterActive, $onlyWithEvents)->pluck(function(AbstractHandler $e, $k)
		{
