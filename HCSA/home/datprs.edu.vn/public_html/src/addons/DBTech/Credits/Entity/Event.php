 ...
 * @property \DBTech\Credits\Entity\Event[] Transfers
...
class Event extends Entity
{
	 ...
	 * @return \XF\Phrase
	 */
	public function getEventTriggerTitle()
	{
	 ...
	 * @return bool
	 */
	public function isValidForUser(\XF\Entity\User $user = null)
	{
	 ...
	 * @throws \Exception
	 */
	public function getEventTriggerHandler($throw = false)
	{
	 ...
	 */
	public function getCalculatedAmount(AbstractHandler $eventTrigger, \XF\Entity\User $user, \ArrayObject $extraParams)
	{
		if (
		...
		if ($eventTrigger->getOption('multiplier') != AbstractHandler::MULTIPLIER_NONE)
		{
			...
			else if (!$extraParams->negate && $this->minaction == 2)
			{
				// stop the action
		...
		if ($amount < 0)
		{
			...
			)
			{
				...
				else if ($positiveAmount > $user->{$this->Currency->column})
				{
					// not enough credits or cant spend this and action is cancelable - show error
