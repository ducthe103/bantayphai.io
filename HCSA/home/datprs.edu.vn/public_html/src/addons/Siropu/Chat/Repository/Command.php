...
class Command extends Repository
{
     ...
     public function getCommandCacheData()
     {
          ...
          foreach ($this->finder('Siropu\Chat:Command')->where('command_enabled', 1)->fetch() AS $command)
          {
               ...
               $values['command_user_groups'] = serialize($command->command_user_groups);
               $values['command_rooms']       = serialize($command->command_rooms);
               $values['command_options']     = serialize($command->command_options);
